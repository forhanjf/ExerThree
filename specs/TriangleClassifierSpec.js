...
describe('TriangleClassifier', function() {
  ...
  it('classifies as isosceles 4,4,6', function() {
    var result = new TriangleClassifier().classify(4,4,6);
    expect(result).toEqual('isosceles');
  });

  // What happens when a side is 0?
  // What about a negative side?
  it('classifies as an illegal entry -1,4,4', function() {
    var result = new TriangleClassifier().classify(-1,4,4);
    expect(result).toEqual('error');
  });

  it('classifies as an illegal entry 3,4,0', function() {
    var result = new TriangleClassifier().classify(3,4,0);
    expect(result).toEqual('error');
  });
});
